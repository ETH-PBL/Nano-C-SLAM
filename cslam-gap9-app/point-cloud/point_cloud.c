...
static uint16_t int_scan_pose_ids[100];
static uint16_t int_scan_pose_cnt = 0;
static icp_points_t ref_scan, cur_scan;

extern uint8_t verbose;

uint16_t pcl_get_scan_pose_count() { return int_scan_pose_cnt; }

int16_t pcl_get_scan_pose_id(uint16_t idx) {
    if (idx >= int_scan_pose_cnt)
        return -1;
    else
        return int_scan_pose_ids[idx];
}
...
void add_scan_pose_id(uint16_t pose_id) {
    check_internal_constraints(pose_id);
    int_scan_pose_ids[int_scan_pose_cnt++] = pose_id;
}

void check_internal_constraints(uint16_t cur_pose_id) {
    float cur_pose[3];
    float ref_pose[3];
    pcl_to_poses(cur_pose, cur_pose_id, 1);
    // if (verbose) printf("Pre LC: %.2f %.2f %.2f\n", cur_pose[0], cur_pose[1],
    // cur_pose[2]);

    for (int16_t i = 0; i < int_scan_pose_cnt; i++) {
        int16_t ref_pose_id = int_scan_pose_ids[i];
        pcl_to_poses(ref_pose, ref_pose_id, 1);

        float distance = euclidean_dist(ref_pose, cur_pose);
        if (cur_pose_id == 1170 && ref_pose_id == 591) {
            printf("distance %.3f\n", distance);
        }
        if (distance < 1.2f) {
            pcl_extract_scan(cur_pose_id, cur_pose_id + 20, &cur_scan, 1400);
            pcl_extract_scan(ref_pose_id, ref_pose_id + 20, &ref_scan, 1400);
            ...
            // if (verbose) printf("%.3f  %d\n", icp_job.error,
            // icp_job.has_nan);
            if ((icp_job.has_nan == 0) && (icp_job.error < 0.01)) {
                if (verbose)
                    printf("Loop closure %d -> %d\n", cur_pose_id, ref_pose_id);
                // printf("Cur %.1f %.1f  Cnt %d\n", cur_pose[0], cur_pose[1]);
                pcl_add_constraint(cur_pose_id, ref_pose_id, icp_res);
                return;
            }
        }
    }
...
void pcl_add_constraint(int16_t from_node, int16_t to_node, float *icp) {
    ...
    constr_counter++;
    // add_scan_pose_id(from_node);
    // add_scan_pose_id(to_node);
}

void pcl_add_external_constraint(constraint c) {
    constraints[constr_counter] = c;
    constr_counter++;
}
...
//     int16_t forbidden[30];
//     int16_t forbidden_cnt = 0;
//     memset(forbidden, 0, sizeof(forbidden));

//     int32_t int_constraints = constr_counter;
//     for (int16_t i=0; i<int_constraints; i++) {
//         int16_t from_id = constraints[i].fromIdx;
//         int16_t to_id = constraints[i].toIdx;
//         if (has_external_constraint(from_id) &&
//         has_external_constraint(to_id)) {
//             forbidden[forbidden_cnt++] = from_id;
//         }
//     }

//     int16_t ext_constraints = external_constraints_count();
//     for (int16_t i=0; i<ext_constraints; i++) {
//         ext_const_info_t c_ext = get_ext_constraint(i);
//         for (int16_t k=0; k<forbidden_cnt; k++) {
//             if (c_ext.cur_pose_id == forbidden[k]) continue;
//         }
//         constraint c = {0};
//         c.fromIdx = c_ext.cur_pose_id;
//         c.toIdx = 0;
//         memcpy(c.z, c_ext.z, 3 * sizeof(float));

//         pcl_add_external_constraint(c);
//     }
// }
...
void pcl_optimize(slam_job_t *job) {
    ...
    int32_t constr_counter_internal = constr_counter;

    // Add external constraints
    int16_t ext_const_cnt = external_constraints_count();
    for (int16_t i = 0; i < ext_const_cnt; i++) {
        constraint c = get_ext_constraint(i);
        constraints[constr_counter++] = c;
        // printf("External from %d to %d: %.3f %.3f %.3f\n", c.fromIdx,
        // c.toIdx, c.z[0], c.z[1], c.z[2]);
    }
    ...
    constr_counter = constr_counter_internal;
